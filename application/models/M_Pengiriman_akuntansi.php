	...
	class M_Pengiriman_akuntansi extends CI_model {
		...
		public function __construct() {
			$this->client = new Client([
				'base_uri'	=> Globals::restServer() . 'api/',
				'time_out'	=> 10,
			]);
		}
		...
		public function getByMasa($bulan, $tahun, $klien='', $start=0, $limit='') {
			...
			$response	= $this->client->request('GET', 'pengiriman_data_akuntansi', ['query' => $get]);
		...
		public function kirim() {
			...
			if(isset($callback)) {
				...
				if($format_data == 'Hardcopy') {
					...
					$response	= $this->client->request('POST', 'pengiriman_data_akuntansi', ['form_params' => $row]);
					$result		= json_decode($response->getBody()->getContents(), true);
					return $result['data'];
				} else {
					$row = [
					...
					$response	= $this->client->request('POST', 'pengiriman_data_akuntansi', $row);
					$result		= json_decode($response->getBody()->getContents(), true);
		...
		public function konfirmasi($id_data, $status) {
			...
			$response	= $this->client->request('PUT', 'pengiriman_data_akuntansi', ['form_params' => $row]);
			$result		= json_decode($response->getBody()->getContents(), true);
			return $result['status'];
		}
