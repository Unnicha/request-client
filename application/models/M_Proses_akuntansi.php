<?php
	...
	class M_Proses_akuntansi extends CI_model {
		...
		public function getByMasa($status, $bulan, $tahun, $klien='', $start=0, $limit='') {
			...
			$response	= $this->client->request('GET', 'proses_data_akuntansi', ['query' => $get]);
		...
		public function simpanProses() {
			...
			$response	= $this->client->request('POST', 'proses_data_akuntansi', ['form_params' => $row]);
			$result		= json_decode($response->getBody()->getContents(), true);
		...
		public function ubahProses() {
			...
			$response	= $this->client->request('PUT', 'proses_data_akuntansi', ['form_params' => $row]);
			$result		= json_decode($response->getBody()->getContents(), true);
			return $result['status'];
		}
