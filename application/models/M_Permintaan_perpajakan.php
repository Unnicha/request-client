<?php
	...
	class M_Permintaan_perpajakan extends CI_model {
		...
		public function getByMasa($bulan, $tahun, $klien='', $start=0, $limit='') {
			...
			$response	= $this->client->request('GET', 'permintaan_data_perpajakan', ['query' => $get]);
		...
		public function tambahPermintaan() {
			...
			$response	= $this->client->request('POST', 'permintaan_data_perpajakan', ['form_params' => $data]);
			$result		= json_decode($response->getBody()->getContents(), true);
		...
		public function ubahPermintaan() {
			...
			for($i=0; $i<count($id_data); $i++) {
				...
				if(isset($format_data[$i])) {
					...
					$response	= $this->client->request('PUT', 'permintaan_data_perpajakan', ['form_params' => $row]);
					$result		= json_decode($response->getBody()->getContents(), true);
					// return $result['status'];
				} else {
					$row = [
					...
					$response	= $this->client->request('DELETE', 'permintaan_data_perpajakan', ['form_params' => $row]);
					$result		= json_decode($response->getBody()->getContents(), true);
					// return $result['status'];
				}
			}
